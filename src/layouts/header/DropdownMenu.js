...
function DropdownMenu({ open, onClose }) {
  const { logout, user: { profileImage, firstName, lastName, id } } = useAuth();
  return (
    <ul 
      className={`dropdown-menu end-0 px-2 mt-1 border shadow-sm rounded-xl w-sm-90${ open ? ' d-block' : ''}`} 
    >
      <li>
        <Link
          to={`/profile/${id}`}
        ...
        >
          ...
          <div className="d-flex flex-column">
            <span className="text-black fw-bold">{firstName} {lastName}</span>
